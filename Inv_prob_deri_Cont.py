...
#Label encode the Pore Shape column (Categorical variable)
#label_encoder = preprocessing.LabelEncoder()
#df['Pore.shape']= label_encoder.fit_transform(df['Pore.shape'])
...
#Y1_class = np_utils.to_categorical(Y1_class)
...
model.compile(optimizer='adam', loss={'y1_output':'sparse_categorical_crossentropy','y2_output':tensorflow.keras.losses.Huber(delta=1), 'y3_output':tensorflow.keras.losses.Huber(delta=2.5)}, metrics={'y1_output':'accuracy'})
...
predicted_NOPX = predictions[1]
predicted_NOPY = predictions[2]
...
ax1.scatter(y_test[1],predicted_NOPX, edgecolors=(0, 0, 0.5))
...
ax2.scatter(y_test[2],predicted_NOPY, edgecolors=(0, 0, 0.5))
...
print('r2 score for NOP_X :',r2_score(y_test[1],predicted_NOPX))

print('r2 score for NOP_Y :',r2_score(y_test[2],predicted_NOPY))



residuals_NOPX  = y_test[1] - predicted_NOPX.ravel()
fig4 = plt.figure()
ax4 = fig4.add_subplot(111)
ax4.scatter(predicted_NOPX,residuals_NOPX, edgecolors=(0, 0, 0.5))
